# 0110 - Distance and similarity metrics, and the metric axioms
...
for the covariance matrix and the multivariate Gaussian. 0107 for the four ways KL fails to be a
...
2. **Orientation figure.** M03's norms and angles into "this page: which one, and when each lies"
   into 0111 (cosine in an embedding space), 0112 (kernels) and 0113 (what high dimension does to
   all three).
3. **The metric axioms**, which is what this page owns. A metric needs `d(x,y) = 0` if and only if
   ...
   is not**, and **KL is not** in three separate ways. This is the page that files 0107's four
...
`code/0110-distance-metrics.py` against `m10_signals.csv`. Computes all three distances for every
...
- Aggarwal, Hinneburg and Keim, ICDT 2001, section 1, for the `L_k` family. Used mainly on 0113.
