...
9001 (entropy), 9004 (mutual information, and the plug-in bias sweep this page pays off).
...
2. **Orientation figure.** 9004's mutual information as an abstract quantity into "this page: the
...
5. **The identity, key callout, from Quinlan's footnote 3 verbatim:** "maximizing the gain is
   ...
   class." The reader has now met the same number three times: as a KL in 9004, as a
   conditional-entropy reduction in 9004, and as a tree split here. **Named theorem, see proof.**
...
`gain(A) = H(Y) - H(Y | A)`, and page 9004 proved that this equals `I(A; Y)`.
...
4. **`svg.chart`, the bias.** Measured gain against the number of distinct values for a column
   independent of the label by construction, using 9004's sweep, with the honest columns marked as
...
`code/9010-information-gain.py`, carrying Quinlan's fourteen rows inline and loading
