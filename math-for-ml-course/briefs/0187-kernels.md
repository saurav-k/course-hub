# 0187 - Kernels, and why a kernel matrix must be positive semi-definite
...
0185 (distances, and the RBF kernel is a function of one). M03 for the dot product. **M04 owns
positive semi-definiteness and the eigenvalue test**; this page uses it and does not re-derive it.
0188 is *not* a prerequisite, but the page should note it comes next and that adding dimensions on
...
7. **The catalogue, three entries with what each assumes.** Linear `x.x'`. Polynomial
   ...
   `|x - x'|`, so **it is 0185's Euclidean distance wearing a different hat** and everything 0188
...
`code/0187-kernels.py` against `m10_signals.csv` plus a ring problem generated in the file. Verifies
