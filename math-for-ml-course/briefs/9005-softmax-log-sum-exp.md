...
9002 (cross-entropy, which this page produces the `q` for). M01 for the exponential and for the
...
2. **Orientation figure.** M03's dot product produces logits, into "this page: logits become a
   probability vector, safely", into 9002 (which scores it), 9011 (which reports it) and M06
...
`code/9005-softmax-log-sum-exp.py` against `m10_classifier.csv`. Naive against shifted in float64
