# 0186 - Cosine similarity and where it is the wrong tool
...
0185 (metric axioms, and the `|x_hat - y_hat|^2 = 2 - 2cos` relation). 0183 for pointwise mutual
...
2. **Orientation figure.** A `mindmap` is allowed for an orientation figure and suits this page,
   because it is the one that indexes what the rest of the module built: "cosine (0185)",
   "PMI (0183)", "SVD (M04)", "concentration (0188)" as four branches into "reading a retrieval
   score honestly".
3. **What cosine is, and what it throws away.** The definition, then the ranking split: query
   ...
   0.8165 and Euclidean 1.0000. Opposite orders. Then the repair from 0185: normalise and the two
...
7. **The loop this page closes, key callout.** Levy and Goldberg 2014 prove that skip-gram with
   ...
   matrix: 0183 defined PMI, M04 defined the factorisation, and they meet here.
...
**Do not do here:** the curse of dimensionality (0188 owns it), ANN index structures, training
...
5. **`flowchart LR`, the factorisation loop.** Corpus co-occurrence counts -> PMI matrix (0183) ->
...
`code/0186-cosine-similarity.py` against `m10_embeddings.csv`. Cosine from the definition against
