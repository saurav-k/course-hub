...
9003 (KL). M07 for independence and joint distributions. M08 for covariance and correlation, so
...
2. **Orientation figure.** 9003's "KL between any two distributions" into "this page: KL between
   the joint and the product of the marginals" into 9010 (a decision tree split) and 9007 (the PMI
...
5. **Non-negativity comes free**, because it is a KL: 9003 already proved it. **One-way cross-link,
...
6. **Pointwise mutual information**, `PMI(x,y) = log2 (p(x,y) / (p(x)p(y)))`, with the note that
   PMI can be negative while its expectation under the joint cannot. Page 9007 needs this definition.
...
the third form. For the second, substitute the chain rule `H(X,Y) = H(X) + H(Y|X)`, which page 9001
...
so Gibbs from 9003 applies directly, and the equality case of Gibbs says `p(x,y) = p(x)p(y)`
...
distinct values, and page 9010 shows the same effect wrecking a decision tree.
...
`code/9004-mutual-information.py` against `m10_signals.csv`. Computes `I` all three ways and
