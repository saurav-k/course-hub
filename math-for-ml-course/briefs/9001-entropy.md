# 9001 - Entropy: how surprised you should expect to be
...
8. **Trade-off, in the same section.** Entropy is a property of a distribution, not of a sample.
   ...
   One line, forward-link to page 9004, which measures the bias.
...
That last expression is `KL(p || u)`, which page 9003 proves is non-negative and zero only when
...
result page 9003 proves, and 9003 does not use this one, so there is no circle.*
...
`code/9001-entropy.py` against `datasets/m10_signals.csv` and `datasets/m10_classifier.csv`.
