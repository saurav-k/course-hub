# 0180 - Entropy: how surprised you should expect to be
...
8. **Trade-off, in the same section.** Entropy is a property of a distribution, not of a sample.
   ...
   One line, forward-link to page 0183, which measures the bias.
...
That last expression is `KL(p || u)`, which page 0182 proves is non-negative and zero only when
...
result page 0182 proves, and 0182 does not use this one, so there is no circle.*
...
7. **What changes if** the error rate falls to 0.1 per cent? `H(0.001) = 0.0189` bits, so the rate
...
`-0.002 log2 0.002 = 0.002 * 8.9658 = 0.01793`; total **0.0188 bits**. Note the rare class supplies
86 per cent of it. (b) Their 99.8 per cent is the accuracy of a constant predictor. Your 0.0188
...
`code/0180-entropy.py` against `datasets/m10_signals.csv` and `datasets/m10_classifier.csv`.
