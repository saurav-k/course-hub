...
9001 (entropy), 9002 (cross-entropy). M05 for convexity as a picture; this page uses Jensen's
...
2. **Orientation figure.** 9002's "what you pay" minus 9001's "what you must pay" into "this page:
   the excess", into 9004 (mutual information) and the KL term in an RLHF objective.
...
5. **Why "distance" is the wrong word**, as four separate failures, one line each: not symmetric;
   ...
   not length. Page 9006 owns the metric axioms and this page hands it the four failures.
...
`code/9003-kl-divergence.py` against `m10_classifier.csv` and `m10_signals.csv`. Computes KL from
