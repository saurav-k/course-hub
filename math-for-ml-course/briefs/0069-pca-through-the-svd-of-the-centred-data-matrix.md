# 0069 - PCA, derived through the SVD of the centred data matrix
...
- `0064` positive semidefiniteness
...
3. Define the sample covariance matrix `S = (1/(n-1)) X_c^T X_c`, and note from `0064` that it is symmetric positive semidefinite for free, so `0062` applies to it and an orthonormal eigenbasis exists.
...
9. **What is not pinned down**, promised on `0060` and `0062` and paid off here: if `v` is a principal direction so is `-v`, so two runs, two libraries or two machines may hand you opposite signs and both are right. Any code that branches on the sign of a loading is wrong. Where two eigenvalues are equal, the entire plane they span is an eigenspace and even the directions are not determined.
...
`V` is orthogonal and the middle factor is diagonal, so this **is** a spectral decomposition of `S` in the sense of `0062`. Reading it column by column, `S v_i = (sigma_i^2 / (n-1)) v_i`.

*The step that does the real work:* `U^T U = I` collapsing in the middle. It is the same cancellation as `0063`'s principal-axes proof, and it is available for the same reason: the factor is orthogonal. Note what the proof did **not** need: no derivatives, no constrained optimisation, no Lagrange multiplier. That absence is why this page can sit before M05.
...
**The honest boundary.** Result 2 shows that the eigenvector subspace achieves an error of `sum_{j>M} lambda_j`. That no *other* `M`-dimensional subspace does better is the genuine optimality claim, and it is `0068`'s Eckart-Young theorem applied to `X_c`, which the page cites rather than reproving. The variational statement, that `v_1` maximises `b^T S b` over unit `b`, is true, is equivalent, and belongs to M06 by the ownership split above.

## Planned figures

1. **Orientation figure**, `flowchart LR`: `Spectral theorem (0062)`, `The SVD (0066)` and `Eckart-Young (0068)` feed `THIS PAGE - PCA is the SVD of the centred data`, which feeds, out of module, `The variational route (M06)`, `Covariance as a statistical object (M08)` and `Curse of dimensionality (M10)`.
...
4. **Eigenvalues.** For a matrix `[[a, b], [b, a]]` the eigenvalues are `a + b` and `a - b`, so `6.4` and `0.6`, with eigenvectors `(1,1)/sqrt(2)` and `(-1,1)/sqrt(2)`. Perpendicular, as `0062` requires.
...
`code/0069-pca-through-the-svd.py`. Dataset `datasets/spectra.csv`.
