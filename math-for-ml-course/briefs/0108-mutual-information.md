...
0107 (KL). M07 for independence and joint distributions. M08 for covariance and correlation, so
...
2. **Orientation figure.** 0107's "KL between any two distributions" into "this page: KL between
   the joint and the product of the marginals" into 0114 (a decision tree split) and 0111 (the PMI
...
5. **Non-negativity comes free**, because it is a KL: 0107 already proved it. **One-way cross-link,
...
6. **Pointwise mutual information**, `PMI(x,y) = log2 (p(x,y) / (p(x)p(y)))`, with the note that
   PMI can be negative while its expectation under the joint cannot. Page 0111 needs this definition.
...
the third form. For the second, substitute the chain rule `H(X,Y) = H(X) + H(Y|X)`, which page 0105
...
so Gibbs from 0107 applies directly, and the equality case of Gibbs says `p(x,y) = p(x)p(y)`
...
distinct values, and page 0114 shows the same effect wrecking a decision tree.
...
`code/0108-mutual-information.py` against `m10_signals.csv`. Computes `I` all three ways and
