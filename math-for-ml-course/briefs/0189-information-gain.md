...
0180 (entropy), 0183 (mutual information, and the plug-in bias sweep this page pays off).
...
2. **Orientation figure.** 0183's mutual information as an abstract quantity into "this page: the
...
5. **The identity, key callout, from Quinlan's footnote 3 verbatim:** "maximizing the gain is
   ...
   class." The reader has now met the same number three times: as a KL in 0183, as a
   conditional-entropy reduction in 0183, and as a tree split here. **Named theorem, see proof.**
...
`gain(A) = H(Y) - H(Y | A)`, and page 0183 proved that this equals `I(A; Y)`.
...
4. **`svg.chart`, the bias.** Measured gain against the number of distinct values for a column
   independent of the label by construction, using 0183's sweep, with the honest columns marked as
...
`code/0189-information-gain.py`, carrying Quinlan's fourteen rows inline and loading
