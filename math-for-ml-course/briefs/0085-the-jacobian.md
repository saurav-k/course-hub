# 0085 - The Jacobian is the chain rule when both ends are vectors
...
| Label | `core` |
| Prerequisites | 0081, 0083. M03: matrix multiplication, shapes, the matrix as a linear map. |
| Enables | 0086, 0087, 0091, and M08's change of variables |
...
>
> The scalar chain rule of 0081 is the case `m = n = p = 1`, where the matrix product is
...
1. **Orientation, `flowchart LR`.** "The chain rule for scalars (0081)" into "THIS PAGE:
   the chain rule for vectors" into "backpropagation (0086)" and "change of variables (M08)".
