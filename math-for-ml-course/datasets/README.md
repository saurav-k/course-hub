...
| `requests.csv` | 25,000 | 1.3 MB | M07 probability, end to end | `arrival_s` is a Poisson process at 2.3 per second, so counts per window are Poisson and gaps are exponential from one column. `retries` is Geometric(0.85) on the trials-until-success convention, so its support starts at 1 and the other convention is off by exactly one. `cache_hit` is independent of `verified_user` and `flagged` depends on it, so a real independence and a real dependence sit in one file. `flagged` is 99 rows in 25,000, rare enough that a 92-percent-recall detector still has 24 percent precision. `latency_ms` is 97% normal with a heavy 3% tail, so the normal fits the bulk and misses the tail by 6.7 times. |
