...
| 10 | **Lecture 7: the shapes, spent on decisions** | written | Lessons `0096` to `0109`: a hub plus thirteen parts, plus the Lecture 7 formula sheet in `reference/`. Six pages of class notes taken during the session, dated **Saturday 5 September 2026**, alongside the full recording of the same session. The applied pass over the material row 9 introduced from pre-class notes: the polling example worked as a binomial at three sample sizes, the recommendation-system PMF with two CDF questions, then continuous random variables defined from the density with the uniform, the exponential and the Gaussian, each with its application, and the memorylessness proof row 9 left as an exercise. It follows Lecture 6 directly, which is both its true position and the chronological one. **Two things to know before adding to this lecture.** First, the numbering drift recorded in row 9 does not apply here: this lecture's source is the attendee's own class notes, headed "Lecture 7", which agrees with this course's sequence, so no reconciliation is needed. Second, **two of the three error probabilities the session quotes belong to a different sample size than the one written beside them** - `.25` is the value at n = 20 rather than 21, and `.03` is the value at n = 100 rather than 101 - which a student challenged live and the lecturer acknowledged; lesson `0098` carries the recomputation and the hub page names it in advance. |
| 11 | **Lecture 8: expectation, variance and covariance** | written | Lessons `0110` to `0119`: a hub plus nine parts, plus the Lecture 8 formula sheet in `reference/`. Nine pages of the lecturer's handwritten **pre-class** notes, headed Expectation and Moments of RV, carrying **no date and no lecture number**; the position is this course's own next free one and must not be read as the lecturer's numbering, which row 9 records as already drifted. Expectation from the frequency argument, five means worked by three different routes, the Cauchy whose mean is undefined and the Pareto whose mean can be infinite, variance as a second moment with its identity proved, the expected-value rule and linearity, the indicator trick, then covariance and the failure of the converse, closing on the two cases where uncorrelated does imply independent. **Three results are exercises in the source** - the geometric mean, the Poisson mean and the Gaussian variance - and all three are worked in these pages under this course's own name; **the source states no numbers at all**, so every decimal in this lecture is this course's arithmetic. It follows Lecture 7 directly, which is its true position. |
| 12 | **Third tutorial session** | reserved | No source deck exists yet, so nothing is named or anchored. The position after the current material is held, and the exact anchor is pinned in this file and in `index.html` on the day its deck arrives. |
