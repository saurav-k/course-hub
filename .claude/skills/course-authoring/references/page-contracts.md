...
The **name** is what this map calls the lesson. It is echoed into the sidebar rail, which `gen_outline.py` generates from this file, and into the `.ttl` of every pager pointing at that lesson. Those three are one object written down three times and they must agree: edit one and a reader clicks a link expecting one page and lands on a differently named one, while every link still resolves so nothing else catches it.

The **claim** is the page's own `<h1>`, which item 2 of the content-page contract below requires to be "the one idea, phrased as a claim with a verb in it rather than a topic". **A claim is not a name, and nothing compares them.** Requiring them equal would force the `h1` to stop being a claim or the card to stop being a name.

`scripts/check_titles.py` enforces the name, and `validate_site.py` calls it so it gates in CI. A **pager** label and the name may differ by truncation in either direction, because the control is narrow and a map sometimes carries the shorter form; one must be a prefix of the other once both are reduced to lower-case alphanumerics. A **rail** entry gets no latitude at all, because it is generated from this file, so any difference means a stale `outline.js`.
