...
- **The teaching bar.** [`references/pedagogy.md`](references/pedagogy.md) applies unchanged: the learning contract under `.paper-meta`, the orientation figure before the first body section, 1,800 prose words per page, 400 prose words per figure, two diagram kinds, the retrieval-practice quiz, a practice problem with a revealable solution, and the recap with its next step. A page the reader cannot orient from is the same failure here as it is in the hub.
...
- **The five per-course instruction files** - `AGENTS.md`, `CLAUDE.md`, `MISSION.md`, `PLOT.md` and `NOTES.md`, plus a `BUILDER-SPEC.md` and `RESOURCES.md` where a course carries them. There is no course to carry contracts and reading orders from. Settle the scope, the learner, and the reading order with the user in conversation before drafting, and write the answers into a short `README.md` beside the pages if the pack is more than two files.
