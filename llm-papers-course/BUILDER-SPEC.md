...
2. **Diagrams everywhere** - minimum 4 Mermaid diagrams per lesson, ideally 5-7. Use `flowchart`, `sequenceDiagram`, `stateDiagram-v2` as fits. Every diagram in a `<figure class="diagram"><div class="mermaid">...</div><figcaption>...</figcaption></figure>`. Every figcaption must explain the diagram in plain grammatical English and bold the key takeaway. Inside a quoted Mermaid label, never write `&quot;` for a double quote: the browser decodes the HTML entity before Mermaid parses the block, the bare `"` closes the label early, and the whole diagram renders as Mermaid's "Syntax error in text" graphic. Use Mermaid's own escape `#quot;` instead. For the same reason, **write a line break inside a Mermaid label as the entity `&lt;br/&gt;`, and any `<i>` or `<b>` in a label the same escaped way**: a literal tag is parsed into a real element, the shared runtime stashes the graph source as `textContent` so it can repaint on a palette change, and `textContent` drops the element - so the diagram is right on first paint and mangled the moment the reader touches the appearance controls. A semicolon inside a label is a statement separator and breaks the diagram; use a dash.
...
6. Self-contained HTML. Same `<head>` as the template, in this exact order: `<link rel="stylesheet" href="../../assets/hub.css">`, the mermaid CDN tag, `<script src="../../assets/hub.js"></script>`, `<script src="../outline.js"></script>`. Same spine nav. Nothing at the end of body: the shared runtime mounts the rail, the appearance controls, the quiz widget and the copy buttons by itself. This course owns no CSS and no JS of its own.
