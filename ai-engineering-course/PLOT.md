...
| # | Position | Status | Notes |
|---|---|---|---|
| 1020 | The capstone: the agent that knows when to stop | written | **Read here**, at the foot of module 3. A turn ceiling, a token ceiling, retrieval bound as a tool, a file-backed memory with a traversal test, and an eval set with expected trajectories. The integrator writes it, not a module writer. |
...
| `reference/troubleshooting.html` | **written** | The number did not move. What to check, in order, when a change that should have helped did not. The self-paced conversion of the syllabus's office hours. |
| `reference/figures.html` | **written** | The two figures the course reuses, each copied verbatim from the lesson that owns it: the four request shapes from 0600 and the six-question design review from 0730. |
