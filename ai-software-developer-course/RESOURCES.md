...
- [Claude Code on the web](https://code.claude.com/docs/en/claude-code-on-the-web) - the isolated-VM architecture, the credential and GitHub proxies, teleport, the 100 MB local-bundle ceiling, environment expiry, and the rate-limit statement quoted in `0800` and `0840`.
- [Configure cloud environments](https://code.claude.com/docs/en/cloud-environments) - the four network access levels, the fourteen-row carry-over table `0800`'s chart counts, the published machine size, the setup-script constraints and the environment cache. `0800` and `0850`.
- [Claude Code GitHub Actions](https://code.claude.com/docs/en/github-actions) - interactive against automation mode, the two actor checks, the scheduled-workflow behaviour, and the two cost ceilings. The whole of `0820` and half of `0830`.
- [Claude Code in Slack](https://code.claude.com/docs/en/slack) - whose account a channel session runs under, the one-pull-request limit, and the warning about following directions from other messages. `0820` and `0850`.
- [`anthropics/claude-code-action` examples](https://github.com/anthropics/claude-code-action/tree/main/examples) - MIT. `examples/claude.yml`, `examples/issue-triage.yml` and `examples/ci-failure-auto-fix.yml` are reproduced verbatim in `0820` and `0830`, the last one with its own security comment block intact.
- The six cloud agents in `0810`, each read only from its own documentation: [GitHub Copilot coding agent](https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent) (the 59-minute ceiling and the one-branch scope), [Cursor cloud agents](https://cursor.com/docs/cloud-agents), [Codex cloud](https://learn.chatgpt.com/docs/cloud), [Devin](https://docs.devin.ai/get-started/devin-intro) and [its billing page](https://docs.devin.ai/admin/billing), [Jules](https://jules.google/docs) and [its usage limits](https://jules.google/docs/usage-limits/). **Only Jules publishes concurrency numbers**, which is the evidence that `0810` is written on the mechanism axis rather than the product axis.
- [Linear - agents](https://linear.app/developers/agents) - `actor=app`, the two agent scopes with their documented descriptions, delegation against assignment, and the ten-second acknowledgement. `0820`.
- [git-worktree](https://git-scm.com/docs/git-worktree) - the linked-worktree definition, the one-branch refusal and the clean-tree condition on `remove`. `0840`.
- [GitHub - managing a merge queue](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/configuring-pull-request-merges/managing-a-merge-queue) - temporary branches, the `merge_group` requirement, removal and rebuild, and both 1-to-100 ranges. `0840`.
- [GitHub - billing for GitHub Actions](https://docs.github.com/en/billing/concepts/product-billing/github-actions) - the $0.006 per minute standard Linux runner rate behind `0840`'s cost calculator.
- [Cloudflare - Agents SDK overview](https://developers.cloudflare.com/agents/), [the API reference](https://developers.cloudflare.com/agents/api-reference/agents-api/), [build a chat agent](https://developers.cloudflare.com/agents/getting-started/build-a-chat-agent/), [remote MCP server](https://developers.cloudflare.com/agents/guides/remote-mcp-server/), [the launch post](https://blog.cloudflare.com/build-ai-agents-on-cloudflare/), [AI Gateway](https://developers.cloudflare.com/ai-gateway/) and [its get-started page](https://developers.cloudflare.com/ai-gateway/get-started/), and [Workers AI pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/) - the whole of `0860`. Cloudflare's documentation is CC BY 4.0 with its code under MIT, and [`cloudflare/agents`](https://github.com/cloudflare/agents) is MIT, so the two example files in `0860` are reproduced with attribution.

### Module 09 - Building an AI-native team

- [MCP - authorization, current revision](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization) and [the superseded one](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization) - the whole of `0910` and its comparison table. The 401 and 403 challenges are reproduced verbatim from the current revision, whose text the MIT-licensed specification project publishes.
...
- [Docker - MCP gateway](https://docs.docker.com/ai/mcp-gateway/) and [the MCP registry](https://github.com/modelcontextprotocol/registry) - the three things people call one word in `0900`: container isolation, credential injection and call tracing on one side, namespace ownership proved by GitHub OAuth, GitHub OIDC, DNS or HTTP on the other. Neither page publishes a configuration schema or a licence for its own text, so `0900` describes both and reproduces from neither.
...
- [Claude Code - connect to an LLM gateway](https://code.claude.com/docs/en/llm-gateway-connect) - the two environment variables and, decisively, which HTTP header each credential variable uses. `0930`.
- [Claude Code - example settings files](https://code.claude.com/docs/en/settings-example) - the organisation `managed-settings.json` reproduced in full on `0950`, with a comment on every key.
- [LiteLLM - proxy reliability](https://docs.litellm.ai/docs/proxy/reliability) - the three fallback lists keyed by error class, which is the part a gateway configuration usually gets wrong. `0930`.
- [Claude Code on Amazon Bedrock](https://code.claude.com/docs/en/amazon-bedrock) - model pinning, and the streamed-response `Content-Type` a proxy must forward unchanged. The concrete instance of "a gateway is infrastructure you operate" on `0930`.
- [Anthropic - how Anthropic secures its AI-native SDLC](https://claude.com/blog/how-anthropic-secures-its-ai-native-software-development-lifecycle) - the 80% of merged code and the 16% to 54% shift in substantively reviewed pull requests. **Self-reported, about the vendor's own engineering**, and labelled that way on `0960`.
- [NBER working paper 35275, Demirer, Musolff and Yang](https://www.nber.org/papers/w35275) - the commit-to-project-to-release attenuation and the 0.25 elasticity of substitution. Cited by `0960` and `0970` as well as by module 10.
- [Replit - Agent 3 announcement](https://replit.com/blog/introducing-agent-3-our-most-autonomous-agent-yet) and [autonomy modes](https://docs.replit.com/replitai/autonomy-level) - the marketing half of `0970`, cited as marketing. The announcement publishes no method behind its 3x and 10x comparisons.
- [Cloudflare - AI Gateway universal endpoint](https://developers.cloudflare.com/ai-gateway/universal/) - the array-order-is-the-fallback-chain idea and the response header naming which step answered. **The endpoint itself is deprecated in favour of dynamic routing**, so `0930` cites the idea and says so.
...
- [Claude Code - GitHub Actions](https://code.claude.com/docs/en/github-actions) - interactive against automation mode, the two actor checks, the required permissions, workload identity federation, and the documented reason CI does not run on commits made with the default token. `1000`, `1010`, `1030`.
- [anthropics/claude-code-action, `examples/`](https://github.com/anthropics/claude-code-action/tree/main/examples) (MIT) - eleven working workflow files. `1000` reproduces `agent-approval-check.yml`, `1010` reproduces `claude.yml` and `issue-triage.yml`, `1030` reproduces `claude-wif.yml`, all verbatim with attribution.
- [Claude Code - memory](https://code.claude.com/docs/en/memory) - also the whole of `1020`: the load order, the `paths` frontmatter, the four automatic-memory types, the 200-line and 25KB index limits, and the sentence that instruction files are context rather than enforced configuration.
- [Claude Code - monitoring usage](https://code.claude.com/docs/en/monitoring-usage) - also `1040`: the eight metrics, the twelve event names, the standard attribute set, and `prompt.id`. The `tool_decision` attribute values are what the audit row on that page is built from.
- [Claude Code - sandboxing](https://code.claude.com/docs/en/sandboxing) - also `1030` and `1040`: the `credentials` block, the `deny` and `mask` modes, `injectHosts`, `tlsTerminate`, `failIfUnavailable`, and how network egress is enforced by a proxy outside the sandbox.
- [Anthropic - when AI builds itself](https://www.anthropic.com/institute/recursive-self-improvement) - the self-reported task-horizon series and doubling claim, the merged-code share, the researcher poll, and the API-error campaign. **Every figure here is self-reported with no published method**; `1020` and `1050` label it as such and say what would make it checkable.
- [Anthropic - what 400,000 sessions say about working with Claude Code](https://www.anthropic.com/research/claude-code-expertise) - the decision split, the session task mix and its shift, and the expertise gap in verified success and abandonment. The whole quantitative half of `1060`.
- [Stack Overflow developer survey, AI section](https://survey.stackoverflow.co/2025/ai) - also `1050`, for adoption against trust and the "almost right, but not quite" frustration.
...
- **Concurrency and rate limits for five of the six cloud agents.** Only Jules publishes them. `0810` is written on the mechanism axis for this reason, states the absence on the page, and leaves four cells of its comparison table reading "not published" rather than filling them from a third party.
- **A per-ACU price for Devin.** The billing page states only that enterprise customers "are billed in Agent Compute Units (ACUs) at the rate set in their order form". `0810` quotes that and prints no number.
- **Cursor's concurrency behaviour.** The documentation says an unlimited number of agents may run in parallel and publishes no ceiling and no queueing behaviour, so `0810` labels it MARKETING in a `.callout.warn` and says what would make it evidence.
...
- **A current issue-benchmark trajectory.** The public leaderboard did not return its table to the author's fetch, so the only measured point available is the 1.96% in the original paper. `1050` prints that point, prints no curve, and says in a warning why not: a line drawn from one measured point and a series of vendor-reported ones is not a measurement.
- **An independent replication of the release attenuation.** The 180-to-30 cascade rests on one paper and one population. `1050` names replication as the evidence that would overturn it rather than treating one study as settled.
- **Any published test of where the gate belongs.** `1000` argues for merge from the structure of the loop and from the attenuation measurement, not from a trial. Nobody has published a comparison of teams that moved the gate downstream, and `1060` records that as the first of its three "what would change my mind" claims.
