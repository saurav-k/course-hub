...
- **Whether this course should join `EXTENDED_BAR_COURSES` in `check_pages.py`** - one practice problem and one hand-drawn `svg.chart` per page.
  **Measured, and it already clears both.** All 83 lessons carry a `.practice` block with a `details.solution` and a `.p-check`, and all 83 carry at least one `svg.chart`; running the whole course with the course name added to that set produces **zero findings**.
  So joining is one line in `check_pages.py` and nothing else, and it costs the course nothing today - it buys the guarantee for the next page written. It is a change to the shared standard rather than to this course, so it is the captain's to take, and it does not belong in a lesson pull request.
